...
(function ($) {
  ...
  var CapacityAges = /*#__PURE__*/function () {
    function CapacityAges() {
      _classCallCheck(this, CapacityAges);
    ...
    _createClass(CapacityAges, [{
      key: "handle",
      value: function handle() {
        var _this = this;

        $('input').each(function (cnt, row) {
          var name = $(row).attr('name'); // 年齢定員カラムは、追加する

          if (name !== undefined && /^age_g\d_(\d)+$/.test(name)) {
            _this.allAgeInputs.push(row);
      ...
      key: "toggleAgeInputsWritable",
      value: function toggleAgeInputsWritable() {
        var _this2 = this;

        this.disableAgeColumns = [];
        this.allAgeInputs.forEach(function (input) {
          /// 未入力の場合は、一緒に使えるカラムも登録
          if ($(input).val()) {
            var name = $(input).attr('name');

            _this2.allAgeInputs.forEach(function (tmpInput) {
              var tmpName = $(tmpInput).attr('name');

              if (name != tmpName && !inArray(tmpName, _this2.allCanUseWithAgeNames[name])) {
                _this2.disableAgeColumns.push(tmpName);
              }
            });
          }
        });
        this.disableAgeColumns.forEach(function (column) {
          _this2.disableInput(_this2.makeInput(column));
        });
        this.kindAgeColumns.forEach(function (column) {
          if (!inArray(column, _this2.disableAgeColumns)) {
            _this2.enableInput(_this2.makeInput(column));
          }
        });

        if (this.capazeroColumns) {
          this.capazeroColumns.forEach(function (name) {
            _this2.disableInput($('input\[name="' + name + '"\]'));
          });
        }
      }
    }, {
      key: "addEvent",
      value: function addEvent(row) {
        var _this3 = this;

        $(row).on('keyup', function () {
          _this3.toggleAgeInputsWritable();
        });
      }
    }, {
      key: "makeInput",
      value: function makeInput(name) {
        return $('input\[name="' + name + '"\]');
      } // disabledにすると、バリデーションエラーの際に、送信時に送られずにバグるため、readonly

    }, {
      key: "disableInput",
      value: function disableInput(input) {
  ...
  var capacityAges = new CapacityAges();
  capacityAges.handle();
})(jQuery);
