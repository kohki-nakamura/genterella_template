        ...
        (function (undef) {
          ...
          main = function main(name, deps, callback, relName) {
            var cjsModule,
                ...
                relParts,
                args = [],
                callbackType = _typeof2(callback),
...
(function ($) {
  ...
  window.CKEDITOR.editorConfig = function (config) {
    ...
    config.toolbar = [{
      name: 'styles',
      items: ['Templates', 'Format']
    }, {
      name: 'link',
      items: ['Bold', 'Blockquote', 'CiteTag', 'Link', 'AdLink', 'Unlink']
    }, {
      name: 'insert',
      items: ['Image', 'Youtube', 'InsertHtml', 'BulletedList', 'NumberedList', 'Table']
    }, {
      name: 'others',
      items: ['InsertPre']
    }, {
    ...
    config.removeButtons = 'Underline,Subscript,Superscript'; // タグを勝手に変換されないようACF（AutoContentFilter）をOFFに

    config.allowedContent = true; // テンプレートの設定ファイルを指定

    config.templates_files = ['/assets/admin/js/articles/templates.js'];
    config.templates = 'default'; // Set the most common block elements.
  ...
  var direct_html = 'div[data-class="direct-html"] {' + 'background-color: #F8F8D8;' + 'border: 1px solid #F8F90F;' + 'padding: 10px;' + 'background-image: url(/images/inserthtml_back.png);' + 'background-repeat: no-repeat;' + 'background-position: 98% 2%;' + '}';
  direct_html += '.cke_editable div[class="button-detail"] a {' + 'font-size: 15px;' + 'line-height: 1.4em;' + 'color: #000080;' + 'text-align: center;' + 'border-radius: 4px;' + 'background-color: #FFF;' + 'width: 340px;' + 'padding: 18px 50px;' + 'display: block;' + 'margin: 40px auto;' + 'position: relative;' + 'box-sizing: border-box;' + 'border: solid 1px #67aad4;' + '}';
  window.CKEDITOR.addCss(direct_html + '.cke_editable h2 { border-bottom: 3px solid #eee } ' + '.cke_editable h3 { border-bottom: 1px solid #eee }' + '.cke_editable img { width: 33%; }');
  window.CKEDITOR.on('dialogDefinition', function (ev) {
    ...
    if (dialogName == 'link') {
      ...
      uploadTab.add({
        type: 'html',
        id: 'captionHtml',
        html: "<p>&nbsp;</p>" + "<p>・サイズ規定なし</p>" + "<p>・推奨は４：３</p>" + "<p>・理想画像は800px x 600px</p>"
      });
    }
  });

  var unloadCallback = function unloadCallback(e) {
    var message = 'このまま別ページへ移動すると、編集中の内容が失われます。';
    e.returnValue = message;
    return message;
  };

  var editor = window.CKEDITOR.replace('body').on('key', function () {
    if (typeof window.onbeforeunload != "function") {
      window.addEventListener('beforeunload', unloadCallback, false);
    }
  });
  $('#article_form input[type="text"],#article_form textarea').keyup(function () {
    if (typeof window.onbeforeunload != "function") {
      window.addEventListener('beforeunload', unloadCallback, false);
    }
  });
  $('#article_form select').change(function () {
    if (typeof window.onbeforeunload != "function") {
      window.addEventListener('beforeunload', unloadCallback, false);
    }
  });
  ...
  $('#body_image_provider_url_save').on('click', function () {
    ...
    save_provider_url(form_value).done(function (data) {
  ...
  $('#article_form').submit(function () {
    ...
    if (status === 'publish') {
      ...
      if (($("input[name='old_status']").val() == 'draft' || $("input[name='old_status']").val() == 'writing' || $("input[name='old_status']").val() == 'proofreading') && $("input[name='article_feeds[]']:checked").length > 0) {
